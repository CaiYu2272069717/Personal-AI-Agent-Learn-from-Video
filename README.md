...
- `@` 工作区文件检索：输入 `@` 实时检索工作目录下的文件/文件夹，支持逐级进入子目录、键盘导航
- 文件上传真实落盘到工作目录：可累加多次上传、逐个删除误传项，并作为工作区引用供 Agent 直接读取
- 可配置 Agent 工作目录：所有文件工具与命令以该目录为基准，Agent 通过系统提示实时感知当前工作目录
...
- **工作目录设置**：在 Agent 对话页右上角的“工作空间”弹窗中可设置工作目录（留空默认 `workspace/`，支持相对项目根或绝对路径）。所有文件工具（`read_file`/`write_file`/`edit_file`/`list_dir`/`glob_files`）与 `run_command` 都以此目录为基准，Agent 传入的相对路径会解析到该目录下。
- **Agent 感知当前目录**：工作目录会实时注入 Agent 系统提示，模型能明确知道“当前目录 / cwd”是哪里；当用户说“当前目录”“这里”时即指该路径。
- **文件上传**：点击输入框的上传按钮可把文件真实保存到工作目录（重名自动追加序号），多次上传会累加而非覆盖，每个附件带删除按钮以便移除误传项。发送时会自动以 `@相对路径` 形式引用，Agent 可直接读取这些文件。
- **`@` 文件检索**：在输入框输入 `@` 会实时检索工作目录下匹配的文件与文件夹，支持 `子目录/前缀` 逐级进入，↑↓ 选择、Tab/Enter 确认、Esc 关闭。
...
当前共 79 个测试，覆盖配置、数据库、链接解析、媒体处理、ASR、总结、知识库、流水线、Agent 后台运行、权限确认、代码沙箱、按轮次 Revert、OCR、设置接口，以及评测模块的 Golden Cases 数量与维度、离线套件指标/Trace/报告生成、对比实验 delta 和指标聚合函数。
